...
const mockUserResponse = {
  ...
  achievements: [
    ...
    {
      name: "Are You an Elitist or Identity Crisis?",
      unlocked: false,
      type: "lifetime",
    },
...
test("Verify initial website loaded with search username input and clickable", async ({
  page,
}) => {
  await page.goto("https://tastecheck.me");
  ...
  await expect(
    page.getByRole("heading", { name: "Roast Me: Your Consent" }),
  ).toBeVisible();
  await page.getByRole("button", { name: "Cancel" }).click();
});

test("Verify user comparison page is being loaded with double search username input", async ({
  page,
}) => {
  await page.goto("https://tastecheck.me/compare.html");
  ...
  await expect(
    page.getByRole("heading", { name: "Achievement Guide" }),
  ).toBeVisible();
  await expect(
