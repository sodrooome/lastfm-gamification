...
This design language was introduced gradually, page by page — not every page has been migrated yet. As of this writing:

| Page | Status |
|---|---|
| `index.html` (landing + dashboard) | ✅ Migrated — ink + coral throughout |
| `compare.html` | ⏳ Not yet migrated — its verdict card and section eyebrows still use the legacy sage green (`{colors.legacy-sidebar-bg}`) |
| `about.html` | ⏳ Not yet migrated — its step-number chips and two of its three feature-card icons still use legacy green/ember tints |
...
When migrating a remaining page, replace `{colors.legacy-sidebar-bg}` / `{colors.legacy-sage-deep}` usage with `{colors.primary}` (for a dark surface) or drop it in favor of the neutral/accent tile pattern described under Components — don't introduce a third color to bridge the gap.
...
### Legacy (being phased out — see Rollout status)

- **Sage** (`{colors.legacy-sidebar-bg}` — #8db87a) and **Sage Deep** (`{colors.legacy-sage-deep}` — #2e5036): The old sidebar/verdict-card green. Still present in `compare.html`'s results verdict card and `about.html`'s step-number chips. Do not use in new work — replace with `{colors.primary}` when you touch those pages.
...
### Don't
- Don't use `{colors.legacy-sidebar-bg}` (sage green) in new work — it's being phased out page by page (see Rollout status).
...
- `compare.html` and `about.html` still contain the legacy green (see Rollout status) — pending migration.
